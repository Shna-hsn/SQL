CREATE OR REPLACE PROCEDURE SP_COLLECTDPMUPPHN_DAILY(PROCESSDATE IN DATE DEFAULT SYSDATE-1, AUTOFLAG IN BOOLEAN DEFAULT TRUE)
IS
   V_DATE DATE;
   ...
   IS
        select period,LINE,mfgtype,EQUIVALENT_output output,presenthour,round((EQUIVALENT_output/presenthour),2)UPPH from (Select
...
WHERE
 ...
 where EQUIVALENT_output>0 and presenthour>0;
...
--²¾°£¥þ½u­pºâµ{¦¡½X 
   /*CURSOR CUR_ALLLINEFIELDLIST(V_DATE DATE) IS
        ...
        DECODE(SUM(A.PRESENTHOUR),NULL,0,ROUND(SUM(A.OUTPUT) /SUM(A.PRESENTHOUR),2)) AS UPPH
        FROM DPMUPPHN A
...
BEGIN
   ...
   IF AUTOFLAG=TRUE THEN
       FOR R_T1 IN CUR_FIELDLIST(V_DATE) LOOP
           ...
           delete from dpmupphn where period=R_T1.PERIOD and  periodtype='daily' and site=v_site and plant=v_plant and mfgtype=R_T1.MFGTYPE and line=R_T1.LINE;
            INSERT INTO DPMUPPHN
              (SYNCID, SYNCDATE, SYNCACTION, PERIOD, PERIODTYPE, SITE, PLANT, CUSTOMER, MFGTYPE, LINE, OUTPUT, PRESENTHOUR, UPPH)VALUES
              (SYNIDUPHN.NEXTVAL, SYSDATE, V_SYNCACTION, R_T1.PERIOD, 'daily', V_SITE, V_PLANT, '*', R_T1.MFGTYPE, R_T1.LINE,  R_T1.OUTPUT, R_T1.PRESENTHOUR, R_T1.UPPH);
           END IF;
       END LOOP;

       /*FOR R_T2 IN CUR_ALLLINEFIELDLIST(V_DATE) LOOP
       delete from dpmupphn where period=R_T2.PERIOD and  periodtype='daily' and site=v_site and plant=v_plant and mfgtype=R_T2.MFGTYPE and line='*';
            INSERT INTO DPMUPPHN
              (SYNCID, SYNCDATE, SYNCACTION, PERIOD, PERIODTYPE, SITE, PLANT, CUSTOMER, MFGTYPE, LINE, OUTPUT, PRESENTHOUR, UPPH)VALUES
              (SYNIDUPHN.NEXTVAL, SYSDATE, V_SYNCACTION, R_T2.PERIOD, 'daily', V_SITE, V_PLANT, '*', R_T2.MFGTYPE, '*',  R_T2.OUTPUT, R_T2.PRESENTHOUR, R_T2.UPPH);
       END LOOP;*/
   END IF;
...
EXCEPTION
   when others then
          ...
          insert into dpmerrlog
               values (sysdate, 'SP_COLLECTDPMUPPHN_DAILY', var_err_msg);
